...
add_action('wp_head','form_css_login');

function form_css_login(){
...
if(get_post_type( $post->ID )=='sfwd-topic'):
	...
	if(!is_user_logged_in()){
		...
		if(get_the_lessontopic($post->ID,854)!=1){
?>	
	<style type="text/css">
		...
		.wp-submit2{
			background: #006fb8 none repeat scroll 0 0 !important;
		...
		.fplearndash-content form table td{
			
			padding:8px 0;
				
		}	
	
		.fplearndash-content form table{
			width: 100%;
		}
...
function get_the_lessontopic($pid,$lessonid){
	
		$swtopicinfo = get_post_meta($pid,'_sfwd-topic', true);
		

		if($lessonid==$swtopicinfo['sfwd-topic_lesson']){
			$var = 1;
			return $var;
